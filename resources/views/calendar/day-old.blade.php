        ...
        <div class="calendar-container">
            <div class="calendar-header ">
                <form action="" class="filter">
                    ...
                    <span style="margin-left: 20px;color: #888">2022</span>
                </form>
            </div>
            <script>
            ...
            <div class="calendar">
                ...
                @forelse($shifts ?? [] as $shift)
                    ...
                    @forelse($shift->orders ?? [] as $order)
                        <section class="task" style="grid-column: {{$loop->parent->iteration + 1}};grid-row: {{Helper::rowTime($order->time_start)}}{{'/'}}{{Helper::rowTime($order->time_finish)}};">
                            <div class="task-1"></div>
                            <div class="task_info">
                                <div class="info_head">
                                    <div class="task_title">{{$order->title}} — {{$order->total}} ₽</div>
                                    <div class="task_time">{{Helper::time($order->time_start)}} - {{Helper::time($order->time_finish)}}</div>
                                    {{--<div class="task_price">Стоимость: {{$order->total}} ₽</div>--}}
                                </div>
                                <div class="info_footer">
                                    <b>Клиент:</b> {{$order->user_name}} @if($order->user_id)
                                        <span class="btn_show_phone" onclick="getContact({{$order->user_id}})">{{Helper::phone($order->user_phone)}}</span>
                                    @endif
                                    <a href="{{route('history.view', ['user_id' => $order->user_id])}}" target="_blank" class="day_history"><i class="fas fa-history"></i></a>
                                </div>
                                <div class="open_order" onclick="viewOrder({{$order->id}})">Детали записи</div>

                            </div>
                            <a href="{{route('orders.create', $order->id)}}" class="task_edit"><i class="fas fa-edit"></i></a>
                            <div class="checkbox">
                                <input class="js_is_completed" type="checkbox" id="checkbox-{{$order->id}}" {{$order->is_completed ? 'checked' : null}} value="{{$order->id}}">
                                <label for="checkbox-{{$order->id}}"></label>
                            </div>
                            <form action="{{route('orders.delete', $order->id)}}" class="task_delete" method="POST">
                                @csrf
                                <input type="hidden" name="shift_id" value="{{$shift->id}}">
                                <input type="hidden" name="order_id" value="{{$order->id}}">
                                <button class="btn_delete_order" onclick="return proverka();"><i class="fas fa-trash-alt"></i></button>
                            </form>
                        </section>
                    @empty
                    @endforelse
